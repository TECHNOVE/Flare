...
public class JfrReader implements Closeable {
    ...
    public final Dictionary<Sample> samples = new Dictionary<>();

    private long nextSampleId = 1;
    ...
    private void readExecutionSample() {
        ...
        long sampleId = this.nextSampleId++;
        samples.put(sampleId, new Sample(time, tid, stackTraceId, threadState));

        StackTrace stackTrace = stackTraces.get(stackTraceId);
        if (stackTrace != null) {
            stackTrace.samples.add(sampleId);
        }
    }
